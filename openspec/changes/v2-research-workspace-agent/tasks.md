...
- [x] 22.7 LLM Proxy 防腐层将 Pi-style tools 归一成 OpenAI function tools，避免真实 provider 收不到 `write_file` / `create_artifact` schema（`@cap/api` provider + pi-runtime test、typecheck 通过）
