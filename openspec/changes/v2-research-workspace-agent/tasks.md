...
- [x] 22.7 LLM Proxy 防腐层将 Pi-style tools 归一成 OpenAI function tools，sandbox tool result 不再 `terminate`，artifact kind 收窄到 `text|code|sheet|image`，避免真实 provider 收不到 schema、首个工具后提前停止或产出非法 artifact kind（`@cap/api` provider + pi-runtime test、typecheck 通过）
