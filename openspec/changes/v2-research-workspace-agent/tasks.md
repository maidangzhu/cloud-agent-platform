...
- [x] 22.7 LLM Proxy 防腐层将 Pi-style tools 归一成 OpenAI function tools，`write_file` 不提前 terminate、`create_artifact` 后 terminate，artifact kind 收窄到 `text|code|sheet|image`，并在 sandbox 内对“已写文件但未产出 artifact”的 run 走同一 ingest/tool-call 路径补建 artifact（`@cap/api` provider + pi-runtime test、typecheck 通过）
