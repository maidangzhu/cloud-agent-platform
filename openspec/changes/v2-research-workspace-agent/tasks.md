...
- [x] 22.7 LLM Proxy 防腐层将 Pi-style tools 归一成 OpenAI function tools，sandbox tool result 不再 `terminate`，artifact kind 收窄到 `text|code|sheet|image`，并在 sandbox 内对“已写文件但未产出 artifact”的 run 走同一 ingest/tool-call 路径补建 artifact（`@cap/api` provider + pi-runtime test、typecheck 通过）
- [ ] 22.8 部署新版 API 后跑公网 production E2E：`https://api.sandbox.maidang.me` 创建 run → 真实 Vercel Sandbox 内 Pi runtime → 真实 provider tool calls → ingest 写 `RunEvent`/`RunToolCall`/`WorkspaceFile`/`WorkspaceArtifact`，并从公网 API 读回验证
