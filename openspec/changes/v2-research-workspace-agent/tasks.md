...
- [x] 22.11 打开并验证 Pi runtime thinking/reasoning：start config 默认 `medium` 并透传 `reasoning_effort`；Control Plane 通过 `LLM_CHANNEL_PI_RUNTIME=2` 选择具备 reasoning 能力的 channel；production API + Vercel Sandbox + real provider gate 证明 thinking delta 在最终语义事件前进入 Redis/SSE，结束后与单条 `agent_thinking` 完全一致并先于 `agent_message`；authenticated browser SSE 断线续传 gate 同时通过
- [x] 22.12 将 Pi runtime LLM proxy 改为真正 streaming：provider SSE parser、LLM Proxy 直接 fan-out、Pi 增量消费、Sandbox 累计 content fallback、Redis `MAXLEN ~10000` 均已部署；2026-07-12 production API + real Vercel Sandbox + real provider gate 证明首批 content 到 Redis 时 `agent_message=0`，长回复完整回放，最终 `agent_message.content` 与全部 content delta 拼接一致
...
- [x] 23.4 收敛 Part 5 Workspace Mapping：Workspace/WorkspaceFile revision + sandbox synced/pending watermark；cold 全量与 live warm 增量 diff；Pi boot 覆盖/删除；`run_command` 临时文件不 ingest；ephemeral provider fresh create 清 watermark；production 从 Neon 重新水合 gate 通过（`dpl_JAYkqzC6jNEpbPfj6jVhTeRWMKgZ`）
