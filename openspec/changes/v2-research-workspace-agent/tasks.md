...
- [x] 22.7 LLM Proxy 防腐层将 Pi-style tools 归一成 OpenAI function tools，sandbox tool result 不再 `terminate`，避免真实 provider 收不到 schema或首个工具后提前停止（`@cap/api` provider + pi-runtime test、typecheck 通过）
