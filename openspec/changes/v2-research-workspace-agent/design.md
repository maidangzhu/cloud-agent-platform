...
当前状态：v1（`cloud-agent-platform-mvp`）已归档，`src/app/api/*`、`src/server/*`、`prisma/schema.prisma` 是唯一的现存实现，将被逐步迁移/重写，不是推倒重来——沙箱封装（`src/server/sandbox/`）、LLM fallback 逻辑（`src/server/agent/`）等已验证可用的部分会被复用而非重写。
