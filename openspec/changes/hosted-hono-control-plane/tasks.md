...
- [x] 6.7 LLM Proxy 在 hosted Control Plane 边界将 Pi-style tools 归一成 OpenAI function tools，`write_file` 成功后立即通过 ingest 提升 artifact 并 terminate，artifact kind 收窄到 `text|code|sheet|image`，避免真实 provider 只写文件后 loop 不收敛（provider + pi-runtime test、`@cap/api` typecheck 通过）
