...
- [x] 6.7 LLM Proxy 在 hosted Control Plane 边界将 Pi-style tools 归一成 OpenAI function tools，并修正 sandbox tool result 不能在首个工具后 `terminate`、artifact kind 必须是 `text|code|sheet|image`（provider + pi-runtime test、`@cap/api` typecheck 通过）
