...
- 全程 TDD：确定性逻辑先写测试，LLM 用 faux provider 脚本化；纯逻辑单元测试离线，业务流程一律用集成测试跑通真实 Vercel 沙箱 + Neon，后端全绿才开前端。
- 真实部署到 Vercel + Neon。
...
**D5. 仅 OpenAI 兼容协议；LLM key 缺失时自动回退 faux provider。**
`model.ts` 用 pi-ai 构造 `api: "openai-completions"` 的 model，`baseUrl` 取 `OPENAI_BASE_URL`（指向中转站）、id 取 `LLM_MODEL`、apiKey 取 `OPENAI_API_KEY`；`OPENAI_API_KEY` 为空则 `registerFauxProvider`。不接 Anthropic 原生 provider（中转站统一走 OpenAI 协议即可）。
理由：保证 clone 后无 key 也能跑通 demo 与测试；单协议降低集成面。
